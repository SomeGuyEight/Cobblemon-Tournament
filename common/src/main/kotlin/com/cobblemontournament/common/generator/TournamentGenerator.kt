...
object TournamentGenerator
{
    ...
    ): TournamentData?
    {
        ...
        val tournamentData: TournamentData = when (properties.tournamentType) {
    ...
    ): Set <MatchProperties>
    {
        ...
        var seedIndex = 0
        val getProperties: ( ( Int ) -> PlayerProperties? ) = {
        ...
        //tournamentType: TournamentType ) // TODO when other tournament types implemented
    {
        for ( match in matchProperties ) {
            val currentRound = roundProperties.firstOrNull {
                it.roundID == match.roundID
            } ?: continue // TODO Log?

            val nextRound = roundProperties.firstOrNull {
                it.roundIndex == currentRound.roundIndex + 1
            }
            val victorIndex = if ( nextRound != null ) {
                TournamentUtil.victorNextMatchIndex( match.roundMatchIndex, currentRound.roundIndex, roundProperties.size )
            } else null
            if ( nextRound != null ) {
            ...
            val previousRound = roundProperties.firstOrNull {
                it.roundIndex == currentRound.roundIndex - 1
            }
            if ( previousRound != null ) {
                val ( evenIndex, oddIndex ) = TournamentUtil.previousMatchIndices( match.roundMatchIndex, currentRound.roundIndex )
            ...
            if ( match.playerMap.size == tournamentProperties.teamSize ) {
                ...
                if ( entries.all { it.value == teamIndex } ) {
                    ...
                    for ( ( uuid , _ ) in entries ) {
                        val player = playerProperties.firstOrNull { it.playerID == uuid } ?: continue
