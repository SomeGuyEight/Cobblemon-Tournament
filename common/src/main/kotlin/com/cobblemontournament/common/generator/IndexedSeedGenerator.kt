    ...
    ): IndexedSeedList
    {
        ...
        while (seedPairs.size * 2 < finalSize) {
            ...
            for (i in size until size * 2) {
                var index: Int = -1
                run loop@ { for (ii in 0 until seedPairs.size) {
                    if (seedPairs[ii].first == targetSeed) {
                        index = ii
                        return@loop
                    } }
                }
                seedPairs.removeAt(index) // let it throw for now
        ...
        return finalSort(orderedList,sortStatus)
    }

    private fun finalSort(
