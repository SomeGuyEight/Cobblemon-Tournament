        ...
        return TournamentSubNode.node(
            Commands.literal( HISTORY )
                .then( Commands.argument( TOURNAMENT_NAME, StringArgumentType.word() )
                    .suggests( TournamentNameSuggestionProvider( restrictToPlayer = false, getActive = false ) )
                    // TODO name suggestions when inactive tournaments set up
                    .executes { ctx ->
                        CommandUtil.displayNoArgument(
                            player  = ctx.source.player,
                            nodeKey = "$TOURNAMENT $HISTORY" )
                    }
