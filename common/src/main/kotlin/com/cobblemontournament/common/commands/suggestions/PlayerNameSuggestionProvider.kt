...
class PlayerNameSuggestionProvider : SuggestionProvider<CommandSourceStack>
{
    ...
    ): CompletableFuture<Suggestions>
    {
        ...
        if (entry.key == BUILDER_NAME) {
            val (tournamentBuilder,_) = TournamentManager.getTournamentBuilderByName(entry.value)
            tournamentBuilder?.getPlayerNames()?.forEach { builder.suggest( it ) }
        } else if (entry.key == TOURNAMENT_NAME) {
            val (tournament,_) = TournamentManager.getTournamentByName(entry.value)
            tournament?.getPlayerNames()?.forEach { builder.suggest( it ) }
        }
