...
// the name is not quite long enough...
...
open class MutableObservableSubListIterator<T>(
    ...
    override fun next(): T {
        if (!hasNext()) {
            throw NoSuchElementException("Observable Sub List iterator does not have next element from index $index.")
        }
    ...
    override fun previous(): T {
        if (!hasPrevious()) {
            throw NoSuchElementException("Observable Sub List iterator does not have previous element from index $index.")
        }
        return subList[--index].also { removedOrAddedThisIteration = false }
    }
    ...
    override fun remove() {
        if (index < 0) {
            ...
            throw IllegalStateException("Observable List iterator 'remove' was called, but 'remove' or 'add' were already called this iteration.")
        }
    ...
    override fun set(element: T) {
        if (index < 0) {
            ...
            throw IllegalStateException("Observable List iterator 'set' was called, but 'remove' or 'add' were already called this iteration.")
        }
